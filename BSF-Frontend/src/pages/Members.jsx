...
export default function Members() {
  ...
  return (
    ...
    >
      {/* Hero section */}
      <div className="relative bg-gradient-to-r from-bsf-green to-bsf-teal rounded-lg sm:rounded-xl md:rounded-2xl mb-8 sm:mb-10 md:mb-16 overflow-hidden">
        <div className="absolute inset-0 bg-grid-pattern opacity-10"></div>
        <div className="absolute top-5 sm:top-10 right-5 sm:right-10 w-20 sm:w-40 h-20 sm:h-40 bg-white/10 rounded-full blur-xl sm:blur-2xl"></div>
        <div className="py-8 sm:py-12 md:py-16 px-6 sm:px-8 relative z-10">
          <motion.div 
            variants={fadeIn}
          ...
          >
            ...
            <p className="text-white/90 text-sm sm:text-base md:text-lg mb-0 max-w-2xl mx-auto">
      ...
      <div className="max-w-6xl mx-auto mb-6 sm:mb-8 md:mb-12">
        <div className="flex justify-center mb-6 sm:mb-8">
          <div className="inline-flex bg-white/50 backdrop-blur-sm p-1 sm:p-1.5 rounded-full shadow-sm">
            {years.map(year => (
              <button
                key={year}
                onClick={() => {
                  setSelectedYear(year);
                  setSelectedMember(null); // Reset selected member when changing year
                }}
        ...
        <h2 className="text-xl sm:text-2xl font-bold text-bsf-green text-center mb-1 sm:mb-2">
          Committee of {selectedYear}
        </h2>
        <p className="text-center text-bsf-gray text-sm sm:text-base mb-6 sm:mb-8">
          {selectedYear === '2025' 
      ...
      <div className="max-w-7xl mx-auto">
        {/* Mobile view: Selected member detail at top */}
        {selectedMember && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 sm:gap-6 md:gap-8">
          ...
          <div className="md:col-span-8 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
            {members.map((member, index) => (
              <motion.div
                ...
                className={`group cursor-pointer ${
                  selectedMember === member ? 'ring-2 ring-bsf-green ring-offset-2 rounded-lg sm:rounded-xl' : ''
                }`}
              ...
              >
                <div className="relative overflow-hidden rounded-lg sm:rounded-xl mb-2 sm:mb-3">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-bsf-green/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center p-2 sm:p-4">
                    <div className="flex space-x-2 sm:space-x-3">
                      <a href={member.social.facebook} className="bg-white/20 hover:bg-white/40 p-1.5 sm:p-2 rounded-full backdrop-blur-sm">
                        <svg className="w-3 h-3 sm:w-4 sm:h-4 text-white" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M22.675 0h-21.35c-.732 0-1.325.593-1.325 1.325v21.351c0 .731.593 1.324 1.325 1.324h11.495v-9.294h-3.128v-3.622h3.128v-2.671c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.795.143v3.24l-1.918.001c-1.504 0-1.795.715-1.795 1.763v2.313h3.587l-.467 3.622h-3.12v9.293h6.116c.73 0 1.323-.593 1.323-1.325v-21.35c0-.732-.593-1.325-1.325-1.325z" />
                        </svg>
                      </a>
                      <a href={member.social.twitter} className="bg-white/20 hover:bg-white/40 p-1.5 sm:p-2 rounded-full backdrop-blur-sm">
                        <svg className="w-3 h-3 sm:w-4 sm:h-4 text-white" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723 9.99 9.99 0 01-3.127 1.195 4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z" />
                        </svg>
                      </a>
                      <a href={member.social.linkedin} className="bg-white/20 hover:bg-white/40 p-1.5 sm:p-2 rounded-full backdrop-blur-sm">
                        <svg className="w-3 h-3 sm:w-4 sm:h-4 text-white" fill="currentColor" viewBox="0 0 24 24">
                ...
                <h3 className="text-xs sm:text-sm font-bold text-bsf-green line-clamp-1">{member.name}</h3>
                <p className="text-bsf-gray text-xs line-clamp-1">{member.position}</p>
              </motion.div>
            ))}
          </div>

          {/* Right side: Selected member detail (desktop only) */}
          <div className="hidden md:block md:col-span-4 md:sticky md:top-24 self-start">
            {selectedMember ? (
              <MemberDetailCard member={selectedMember} />
            ) : (
              <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-md p-4 sm:p-6 text-center">
                <div className="bg-gray-100 w-24 h-24 sm:w-32 sm:h-32 mx-auto rounded-full flex items-center justify-center mb-3 sm:mb-4">
                ...
                <h3 className="text-base sm:text-lg font-medium text-bsf-green mb-1">Select a Member</h3>
                <p className="text-xs sm:text-sm text-bsf-gray">
      ...
      {/* Member Detail Card Component */}
      {function MemberDetailCard({ member }) {
        return (
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-white rounded-lg sm:rounded-xl shadow-lg p-4 sm:p-6 overflow-hidden"
          >
            <div className="relative mb-5 sm:mb-6">
              <div className="aspect-square max-w-[120px] sm:max-w-[180px] mx-auto rounded-lg sm:rounded-xl overflow-hidden shadow-md">
                <img 
                  src={member.image} 
                  alt={member.name} 
              ...
              <div className="absolute -bottom-2 sm:-bottom-3 left-1/2 transform -translate-x-1/2 bg-bsf-green text-white text-xs font-bold px-3 sm:px-4 py-0.5 sm:py-1 rounded-full shadow-sm">
                {member.position}
              </div>
            </div>

            <div className="text-center mb-4 sm:mb-6">
              <h3 className="text-lg sm:text-xl font-bold text-bsf-green mb-1">{member.name}</h3>
              <div className="flex items-center justify-center gap-2 text-bsf-gray text-xs sm:text-sm">
            ...
            <p className="text-bsf-gray text-xs sm:text-sm mb-4 sm:mb-6 text-center">
              "{member.bio}"
            </p>

            <div className="flex justify-center space-x-3 sm:space-x-4">
              <a href={member.social.facebook} className="bg-bsf-green/10 hover:bg-bsf-green/20 text-bsf-green p-1.5 sm:p-2 rounded-full transition-colors">
                <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M22.675 0h-21.35c-.732 0-1.325.593-1.325 1.325v21.351c0 .731.593 1.324 1.325 1.324h11.495v-9.294h-3.128v-3.622h3.128v-2.671c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.795.143v3.24l-1.918.001c-1.504 0-1.795.715-1.795 1.763v2.313h3.587l-.467 3.622h-3.12v9.293h6.116c.73 0 1.323-.593 1.323-1.325v-21.35c0-.732-.593-1.325-1.325-1.325z" />
                </svg>
              </a>
              <a href={member.social.twitter} className="bg-bsf-green/10 hover:bg-bsf-green/20 text-bsf-green p-1.5 sm:p-2 rounded-full transition-colors">
                <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723 9.99 9.99 0 01-3.127 1.195 4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z" />
                </svg>
              </a>
              <a href={member.social.linkedin} className="bg-bsf-green/10 hover:bg-bsf-green/20 text-bsf-green p-1.5 sm:p-2 rounded-full transition-colors">
                <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                </svg>
              </a>
            ...
            <div className="mt-4 pt-4 sm:mt-6 sm:pt-6 border-t border-gray-100">
              <h4 className="text-sm sm:text-base font-medium text-bsf-green mb-2">Contact Information:</h4>
              <div className="text-xs sm:text-sm text-bsf-gray">
                <p className="flex items-center mb-1">
                  ...
                  <span className="break-all">{member.name.toLowerCase().replace(' ', '.')}@example.com</span>
                </p>
